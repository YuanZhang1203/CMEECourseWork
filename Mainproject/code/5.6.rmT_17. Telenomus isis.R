...
alp_vec <- 170 -10.4 *(T_vec-273.15)+0.174*(T_vec-273.15)^2

bpk_vec <- B(B_0_bpk,E_bpk,E_D_bpk,T_vec,T_ref_par,T_pk_bpk+273.15,k_par)

z_vec <- 1.08 - 0.0971*(T_vec-273.15)+0.00228*(T_vec-273.15)^2

zJ_vec <- 0.0674 - 0.0056*(T_vec-273.15)+0.000134*(T_vec-273.15)^2
...
plot(T_vec-273.15,r_m_vector,type = "l",col="blue")
NicheW <-c()
...
for(i in 1:(length(T_pk_mm)/2)){
  a_vec_mm <-  170 -10.4 *(T_vec-273.15)+0.174*(T_vec-273.15)^2
  
  zJ_vec_mm <- 0.0674 - 0.0056*(T_vec-273.15)+0.000134*(T_vec-273.15)^2
  
  #these are the same
  bpk_vec_mm <- B(B_0_bpk,E_bpk,E_D_bpk,T_vec,T_ref_par,T_pk_bpk+273.15,k_par)
  kap_vec_mm <- B0_kap*((exp((-E_par/k_par)*((1/T_vec)-(1/T_ref_par))))/(1+(E_par/(ED_par-E_par))*exp((ED_par/k_par)*((1/T_pk_par)-(1/T_vec)))))
  z_vec_mm <- 1.08 - 0.0971*(T_vec-273.15)+0.00228*(T_vec-273.15)^2
...
dr_dkap <-  function(alp,b_pk,k,z,z_J ) alp/(alp * k + alp * z + 1)^2 * (alp * k * z_J + alp *z * z_J + log10((k + z)^(k + z)/b_pk^(k + z))) - (1/(alp * k + alp * z + 1)) * (alp *  z_J + (((k + z)^((k + z) -  1) * (k + z) + (k + z)^(k + z) * log((k + z)))/b_pk^(k + z) - (k + z)^(k + z) * (b_pk^(k + z) * log(b_pk))/(b_pk^(k + z))^2)/((k + z)^(k + z)/b_pk^(k + z) * log(10)))
...
legend(15,-0.07,c("Full","Juvenile traits constant","Adult traits constant"),
