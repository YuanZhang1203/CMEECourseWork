...
r_SP_app <- function(alp,b_pk,k,z,z_J)  -(1/(alp*k+alp*z+1))*(alp*k*z_J+alp*z*z_J+
                                                                log10(  abs((k+z)) ^  abs((k+z))  /b_pk^  abs((k+z))  ))
...
alp_vec <- 140- 8.2 *(T_vec-273.15)+ 0.13*(T_vec-273.15)^2

bpk_vec <-  -59.3 + 5.78*(T_vec-273.15)-0.115* (T_vec-273.15)^2

z_vec <- 0.101- 0.00874 *(T_vec-273.15)+ 0.000229*(T_vec-273.15)^2

zJ_vec <- B_inv(B_0_zJ,E_zJ,E_D_zJ,T_vec,T_ref_par,T_pk_zJ+273.15,k_par)

kap_vec <- 0.0452- 0.00676 *(T_vec-273.15)+ 0.0000612*(T_vec-273.15)^2
...
plot(T_vec-273.15,r_m_vector,type = "l",col="blue",ylab = expression(r[m]),xlab = "Temperature(C)")
NicheW <-c()
...
for(i in 1:(length(T_pk_mm)/2)){
  a_vec_mm <-  140- 8.2 *(T_vec-273.15)+ 0.13*(T_vec-273.15)^2
  
  zJ_vec_mm <- 1/(B0_zJ*((exp((-E_par/k_par)*((1/T_vec)-(1/T_ref_par))))/(1+(E_par/(ED_par-E_par))*exp((ED_par/k_par)*((1/T_pk_mm[i])-(1/T_vec))))))
  
  #these are the same
  bpk_vec_mm <- -59.3 + 5.78*(T_vec-273.15)-0.115* (T_vec-273.15)^2
  kap_vec_mm <- 0.0452- 0.00676 *(T_vec-273.15)+ 0.0000612*(T_vec-273.15)^2
  z_vec_mm <- 0.101- 0.00874 *(T_vec-273.15)+ 0.000229*(T_vec-273.15)^2
  
  rm_vec_mm<-(  (kap_vec_mm+z_vec_mm)  *(log10  (bpk_vec_mm/   abs((kap_vec_mm+z_vec_mm))  )-(a_vec_mm*zJ_vec_mm)))/((a_vec_mm*  (kap_vec_mm+z_vec_mm)  )+1)  
...
f <- expression(-(1/(alp*k+alp*z+1))*(alp*k*z_J+alp*z*z_J+
                                        log( abs(k+z) ^ abs(k+z) /b_pk^ abs(k+z) )))
...
dr_dalp <- function(alp,b_pk,k,z,z_J)  abs((k + z)) /(alp * k + alp * z + 1)^2 * (alp * k * z_J + alp *  z * z_J + log10(  abs((k + z))  ^  abs((k + z))  /b_pk^  abs((k + z)))) - (1/(alp * k + alp * z + 1)) * (k * z_J + z * z_J)

dr_dbpk <- function(alp,b_pk,k,z) (1/(alp * k + alp * z + 1)) * ((k + z)^(k + z) * (b_pk^((k + 
                                                                                             z) - 1) * abs((k + z)) )/(b_pk^(k + z))^2/( abs(k + z) ^ abs(k + z) /b_pk^(k +  z) * log(10)))


dr_dz <- function( alp,b_pk,k,z,z_J)  alp/(alp * k + alp * z + 1)^2 * (alp * k * z_J + alp *
                                                                         z * z_J + log10(  abs((k + z))  ^  abs((k + z))  /b_pk^  abs((k + z))  )) - (1/(alp * k + alp * z + 1)) * (alp * 
                                                                                                                                                                                      z_J + (((k + z)^((k + z) -  1) * (k + z) + (k + z)^(k + z) * log(  abs((k + z))  ))/b_pk^  (k + z) - (k + z)^(k + z) * (b_pk^(k + z) * log( abs(b_pk) ))/(b_pk^(k + z))^2)/((k + z)^(k + z)/b_pk^(k + z) * log(10)))


dr_dzJ <- function(alp,b_pk,k,z )  -((1/(alp * k + alp * z + 1)) * (alp * k + alp * z))


dr_dkap <-  function(alp,b_pk,k,z,z_J ) alp/(alp * k + alp * z + 1)^2 * (alp * k * z_J + alp *z * z_J + log10(  abs((k + z))  ^  abs((k + z))  /b_pk^  abs((k + z))  )) - (1/(alp * k + alp * z + 1)) * (alp *  z_J + (((k + z)^((k + z) -  1) * (k + z) + (k + z)^(k + z) * log(  abs((k + z))  ))/b_pk^ abs((k + z)) - (k + z)^(k + z) * (b_pk^ abs((k + z)) * log( abs(b_pk)) )/(b_pk^ abs((k + z)) )^2)/(  abs((k + z))  ^  abs((k + z))  /b_pk^ abs((k + z)) * log(10)))
...
plot(T_vec-273.15,dr_dbpk_vec,type = "l",col="red",ylab = expression(drm/db[pk]),xlab = "Temprature",ylim = c(-1000,1000))
...
legend("topleft",c("Full","Juvenile traits constant","Adult traits constant"),
